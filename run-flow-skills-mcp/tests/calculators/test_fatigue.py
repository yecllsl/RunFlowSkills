...
def test_fatigue_all_normal_returns_low():
    """所有指标正常 → 低风险."""
    score, level, factors = calc_fatigue_score(hrv_deviation_pct=2.0, tsb=15.0, rpe_trend=[3, 4, 3])
    assert level == "low"
    assert score < 30
...
def test_fatigue_partial_data_returns_moderate():
    """部分数据缺失仍可评估，但降级."""
    score, level, factors = calc_fatigue_score(hrv_deviation_pct=-8.0, tsb=None, rpe_trend=None)
...
def test_fatigue_rpe_trend_rising():
    """RPE 趋势上升 → 风险增加."""
    score1, _, _ = calc_fatigue_score(hrv_deviation_pct=0.0, tsb=10.0, rpe_trend=[3, 3, 3])
    score2, _, _ = calc_fatigue_score(hrv_deviation_pct=0.0, tsb=10.0, rpe_trend=[5, 7, 9])
    assert score2 > score1


def test_fatigue_all_none_returns_low_with_warning():
    """全部数据缺失 → 默认低风险但 factors 含 'insufficient_data'."""
    score, level, factors = calc_fatigue_score(hrv_deviation_pct=None, tsb=None, rpe_trend=None)
    assert level == "low"
